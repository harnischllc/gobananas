...
Go Bananas (full name: "Go Bananas: Banana Scanner") is a native mobile app (iOS + Android) that scans bananas with the camera and classifies their ripeness stage using a custom hue-based classifier. Solo hobby project run under Harnisch LLC. iOS is in TestFlight (Beta Bananas external group with a public join link, plus the internal Team Expo group); latest is build 1.0.0 (14), in TestFlight 2026-06-13 (carries banana facts, the SVG banana-mark branding across icon and splash, the anonymous corrections loop, and the You-tab lede fix). Android is in Google Play internal testing, latest versionCode 14 (Loggerhead Creative dev account); the v14 internal release is rolled out to internal testers. PUBLIC LAUNCH IS IN PROGRESS as of 2026-06-13: Play production submitted and in review (held on approval, managed publishing now ON); iOS v1.0 prepped but not yet submitted (blockers: review-contact phone, a 13-inch iPad screenshot, Content Rights). Detail in the Open questions section and Open Brain captures 717 and 718.
...
- App Store public release (IN PROGRESS 2026-06-13): v1.0 prepped, NOT yet submitted. Done by me: App Privacy published (anonymous diagnostics, not linked, no tracking, Analytics + App Functionality), age 4+, category Food & Drink, Pricing Free across 175 regions, availability all countries, build 14 attached, sign-in requirement off, MANUAL release selected. Three blockers remain before Add for Review: review-contact phone number, a 13-inch iPad screenshot (REQUIRED because `ios.supportsTablet` is true; iPad shots are NOT optional), and Content Rights Information in App Information. Then export compliance = No and submit.
- Play Store public release (IN PROGRESS 2026-06-13): production release with build 14 submitted, changes IN REVIEW. Managed publishing is now ON, so approved changes HOLD until a manual Publish (no auto-go-live). Data safety updated to declare the corrections (App activity > Other actions, optional, App functionality + Analytics, encrypted in transit, no deletion path). Eric finished the Play Console steps himself.
- v1.1 rewards loop: LIVE in the shipped app (daily crate, streak, collection, the home RewardsCard), NOT `__DEV__`-gated. Only the dev reset buttons inside `mobile/app/rewards.tsx` stay `__DEV__`-gated. Code: `mobile/app/rewards.tsx` + `lib/drops.ts` + `lib/streak.ts` + components. The v2.0 strategic thesis lives in Open Brain under the `v2-ideas` tag (no `memory/project_gobananas.md` file exists).
- Auth: still none (the corrections loop is anonymous, no account). The anonymous corrections loop is now LIVE: opt-in toggle in `you.tsx` plus the result-screen "wrong stage?" picker post to `bananascanner.com/api/corrections` (Cloudflare Worker + D1 `gobananas_corrections`), gated on consent. Privacy policy at bananascanner.com/privacy discloses exactly what is sent. Backend code lives in the bananascanner-website repo.
