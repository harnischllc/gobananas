...
Go Bananas (full name: "Go Bananas: Banana Scanner") is a native mobile app (iOS + Android) that scans bananas with the camera and classifies their ripeness stage using a custom hue-based classifier. Solo hobby project run under Harnisch LLC. iOS is in TestFlight (Beta Bananas external group with a public join link, plus the internal Team Expo group); latest is build 1.0.0 (11), uploaded 2026-06-01 (Apple processing, external Beta review still pending for the Beta Bananas group). Android is in Google Play internal testing, latest versionCode 4 (Loggerhead Creative dev account). 10 of 11 Play Console setup tasks done as of 2026-05-24, only Store listing remains. No public release on either store yet.
