...
Go Bananas (full name: "Go Bananas: Banana Scanner") is a native mobile app (iOS + Android) that scans bananas with the camera and classifies their ripeness stage using a custom hue-based classifier. Solo hobby project run under Harnisch LLC. iOS is live in TestFlight external testing (Beta Bananas group, 3 testers on build 1.0.0 (8), public TestFlight join link active). Android is in Google Play internal testing for versionCode 2 (Loggerhead Creative dev account). 10 of 11 Play Console setup tasks done as of 2026-05-24, only Store listing remains. No public release on either store yet.
