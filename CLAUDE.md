...
- App Store public release: listing copy + 5 iPhone screenshots drafted and saved to the iOS 1.0 version on 2026-06-13. Still open and my call: attach build, App Privacy questionnaire, age rating, pricing (free? free with future paid features?), and the actual submit (Add for Review).
- Play Store: all 11 setup tasks done (Store listing completed 2026-06-13: name, descriptions, icon, feature graphic, 5 phone screenshots; tablet shots are recommended, not required). Privacy URL (bananascanner.com/privacy) is staged in Publishing overview but cannot be sent on its own; it goes to Google bundled inside the single "Send app for review" submit, which stays locked until a release is finished. Managed publishing is OFF, so approved changes auto-publish on approval.
